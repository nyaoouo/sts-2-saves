...
STS2Saves is a Slay the Spire 2 mod for keeping more control over run saves. It archives autosaves, lets you create manual snapshots from the pause menu, and adds a browser for loading, backing up, opening, annotating, and deleting saved run snapshots.

```
This mod is 100% generated by AI
```
...
- Lets you open the folder for the selected archived run or snapshot directly from the browser.
- Lets you attach notes to both archived runs and individual snapshots.
...
- Open the selected snapshot folder or run folder on disk.
- Add or edit a note for the selected run or snapshot.
- Delete a selected snapshot.
- Delete an entire archived run.

When a snapshot is selected, `Open Folder` opens the containing `autosaves/` or `manual/` archive folder for that snapshot. When a run is selected, it opens that run's root archive folder.

Notes are stored with the archive metadata. Snapshot notes apply only to the selected snapshot. Run notes apply to the archived run as a whole and are shown alongside snapshot details in the browser.
...
### Open the archive folder for a save

1. Open the save browser.
2. Select either a snapshot or a run.
3. Press `Open Folder`.
4. The mod opens the matching archive folder in your operating system.

### Add a note to a run or save

1. Open the save browser.
2. Select either a run or a snapshot.
3. Press `Edit Note`.
4. Enter the note text and confirm.

Use run notes for broad context like route plans or test scenarios. Use snapshot notes for moment-specific context like event choices, elite prep, or restore checkpoints.
